...
impl Ppu {
  ...
  fn get_color(&mut self, pixel: u8) -> Color {
    ...
    let palette = if mask_flags.is_rendering() {
      pixel
    } else {
      0
    };
  ...
  fn increment_scroll_x(&mut self) {
    if self.get_mut_registers().mask_flags.is_rendering() {
  ...
  fn log(&mut self) {
    ...
    let fine_x = self.get_mut_registers().fine_x;
  ...
  pub fn clock(&mut self) {
    match self.scan_line {
      ...
      241 => {
        if self.cycles == 1 && !self.get_mut_registers().vblank_suppress {
    ...
    if (2..=255).contains(&self.cycles) || (322..=337).contains(&self.cycles) {
      match self.cycles % 8 {
        ...
        0x04 => {
          ...
          if (self.get_mut_registers().vram_addr.coarse_y() & 0x02) > 0 {
            self.bg_next_tile_attribute >>= 4;
          }
          if (self.get_mut_registers().vram_addr.coarse_x() & 0x02) > 0 {
  ...
  fn evaluate_sprites(&mut self) {
    ...
    for idx in 0..64 {
      let address = idx * 4;
      let sprite = Sprite::new(idx, &self.get_mut_registers().oam_ram[address..address + 4]);

      let sprite_size = usize::try_from(self.get_mut_registers().ctrl_flags.get_sprite_size()).unwrap();
  ...
  fn load_sprites(&mut self) {
    ...
    for sprite in sprites.iter_mut() {
      let scan_line = usize::try_from(self.scan_line).unwrap();
      let tile_address = sprite.tile_address(self.get_mut_registers().ctrl_flags, scan_line);
      sprite.data_lo = self.get_mut_registers().ppu_read(tile_address);
      sprite.data_hi = self.get_mut_registers().ppu_read(tile_address + 8);
    }
  ...
  fn render_sprite_pixel(&mut self, x: usize) -> (u8, bool, bool) {
    ...
    if self.get_mut_registers().mask_flags.is_rendering_sprites(x) {
  ...
  fn render_background_pixel(&mut self, x: usize) -> u8 {
    let mut res = 0;
    if self.get_mut_registers().mask_flags.is_rendering_background(x) {
      let fine_x = self.get_mut_registers().fine_x;
      let nth = 15 - fine_x;
  ...
  fn fetch_next_bg_tile_attribute(&mut self) -> u16 {
    ...
    u16::try_from(self.read_ppu_u8(0x23C0
      | (nametable_y << 11)
      | (nametable_x << 10)
      | ((coarse_y >> 2) << 3)
      | (coarse_x >> 2))).unwrap()
